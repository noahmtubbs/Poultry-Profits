...
function handleSpacebarAction()
    if currentScreen == "farm" then
        ...
        if isInPickupRange(player, shop) then
            ...
            local selectedUpgrade = shop.upgrades[shop.selectedUpgrade]
            if money >= selectedUpgrade.cost then
                money = money - selectedUpgrade.cost
                selectedUpgrade.action()
                player.experience = player.experience + 20
        ...
        for _, chicken in ipairs(chickens) do
            if isInPickupRange(player, chicken) and inventorySystem.hasItem("Special Feed") then
                chicken.goldenEggChance = chicken.goldenEggChance + 5 -- Increase chance
    ...
    elseif currentScreen == "market" then
        -- Deposit eggs at market
        if isInPickupRange(player, marketplace) and player.carryingEggs > 0 then
            ...
            if transferableEggs > 0 then
                ...
                table.insert(messages, {text = "Deposited " .. transferableEggs .. " eggs into the market.", timer = 2})
            else
        ...
        -- Sell eggs to customers
        for i = #customers, 1, -1 do
            local customer = customers[i]
            if isInPickupRange(player, customer) then
                if totalEggsInMarket >= customer.needs then
                    -- Random event: Market demand surge
                    local priceMultiplier = 1
                    if math.random(1, 100) <= 10 then
                        priceMultiplier = 2
                        table.insert(messages, {text = "Market demand surge! Eggs sold for double!", timer = 3})
                    end

                    totalEggsInMarket = totalEggsInMarket - customer.needs
                    local earnings = customer.needs * eggPrice * priceMultiplier * comboMultiplier
                    money = money + earnings
                    table.insert(messages, {text = "Sold eggs for " .. earnings .. " KSH!", timer = 2})
                    ...
                    player.experience = player.experience + 10
                    achievements.masterFarmer.progress = achievements.masterFarmer.progress + earnings
                    checkLevelUp()
